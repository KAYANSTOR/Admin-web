...
function patchSalesList(filePath) {
  ...
  const oldUi = `<div className="font-bold text-\\[14px\\] text-primary-dark">بطاقة: \\{sale\\.cardId \\|\\| 'غير معروف'\\}</div>\\s*<div className="text-\\[12px\\] text-gray-500 flex gap-2">\\s*<span>\\{sale\\.createdAt \\? format\\(new Date\\(sale\\.createdAt\\), 'dd/MM/yyyy HH:mm'\\) : ''\\}</span>\\s*<span>•</span>\\s*<span dir="ltr">\\{sale\\.customerId \\|\\| '-'\\}</span>\\s*</div>`;
...
patchSalesList('src/pages/Sales.tsx');
